...
router.post('/login', async (ctx) => {
    ...
    let token = await LoginManager.adminLogin(v.get('body.username'), v.get('body.password'));
...
router.post('/wxlogin', async (ctx) => {
    const v = await new AdminWxLoginValidator().validate(ctx);
    let token = await LoginManager.adminWxLogin(v.get('body.code'));
    ctx.response.status = 200;
    ctx.body = {
        code: 200,
        msg: '登录成功',
